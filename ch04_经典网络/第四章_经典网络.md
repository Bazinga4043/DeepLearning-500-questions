...
![](./img/ch4/resnet_梯度退化.jpg)

- 通过上图我们可以发现，拥有56层数的网络结构表现明显差于拥有20层数的网络结构，造成这一现象的原因大概有：过拟合、梯度消失/爆炸和深度网络的退化，我们来一一剖析。
- 过拟合：因为过拟合会让网络在训练集上表现得很好，而从上图我们可以看出，无论是在训练集还是测试集中，拥有更深层次的网络表现均比浅层次的网络差，那显然就不是过拟合导致的。
- 梯度消失/爆炸：梯度消失/爆炸是因为神经网络在反向传播的时候，反向连乘的梯度小于1（或大于1），导致连乘的次数多了之后（网络层数加深），传回首层的梯度过小甚至为0（过大甚至无穷大），这就是梯度消失/爆炸的概念。但是可以通过加入一些BN层（Batch Normalize）的操作来规整数据的分布基本解决梯度消失/爆炸的问题，所以这个问题也不是导致深层网络退化的原因。
- 网络退化的主要原因：
​	在MobileNet V2的论文中提到，由于非线性激活函数Relu的存在，每次输入到输出的过程都几乎是不可逆的，这也造成了许多不可逆的信息损失。试想一下，这一部分不可逆的信息损失会随着网络深度的增加慢慢累加，所以这是导致退化的主要原因。

 ### 4.7.2 解决梯度退化的方法--残差结构
 
 - 一种是调整求解方法，比如更好的初始化、更好的梯度下降算法等；另一种是调整模型结构，让模型更易于优化——改变模型结构实际上是改变了error surface的形态。ResNet的作者从后者入手，探求更好的模型结构。在这里举一个简单地例子：如下图所示，为了不让网络退化，可以在网络中加入一些恒等变换的操作，这样随着网络的增加，网络后面层的精度仍然能够和前面层的精度持平，这是Resnet的灵感来源。

![](./img/ch4/resnet_00.jpg)
 - 如上图所示，我们把右边的网络理解为左边浅层网络加深了三层（框起来的部分），假如我们希望右边的深层网络与左边的浅层网络持平，即是希望框起来的三层跟没加一样，也就是加的三层的输入等于输出。
 - 我们假设这三层的输入时x，输出是&H(x)$,那么深层网络和浅层网络表现持平的直观理解是：$H(x)=x$,这种输入等于输出的方式，就是论文中提到的恒等变换(identity mapping)，所以ResNet的初衷，就是让网络拥有这种恒等映射的能力，能够在加深网络的时候，至少能保证深层网络的表现至少和浅层网络持平。

#### 残差结构
 - 事实上，已有的神经网络很难拟合潜在的恒等映射函数$H(x)=x$。但如果把网络设计为$H(x)=F(x)+x$，即直接把恒等映射作为网络的一部分，就可以把问题转化为学习一个残差函数$F(x)=H(x)-x$.只要$F(x)=0$，就构成了一个恒等映射$H(x)=x$。所构成的网络结构如下图所示：
 ![](./img/ch4/basic_block.png)
 - 我们可以看到，残差结构比正常的结构多了右侧的曲线，这个曲线也叫作shortcut connection，通过跳接在激活函数前，将上一层（或几层）的输出与本层输出相加，将求和的结果输入到激活函数作为本层的输出。
 - 我们从数学的角度来看残差结构，假设残差结构的输入为x,则输出$H(x)=F(x,Wi)+x$
 - 其中$F(x,Wi)就是我们要学习的残差，把x移动到等式的左侧，残差就等于$H(x)-x$，残差就是中间有一个RELU激活函数的双层权重。
 
 #### 残差结构的作用
 - 加了残差结构后就是给了输入x多一个选择，在神经网络学习到这层的参数是冗余的时候它可以选择直接走这条“跳接”曲线，跳过这个冗余层，而不需要再去拟合参数使得输出$H(x)=x$
 - 学习残差的计算量比学习输出等于输入小
 - 在反向传播的时候,其梯度值不能消失。

#### short_cut的两种形式：
- 残差网络和普通的卷积神经网络最大的差异之处在于，网络里面多了许多跳级连接的结构，我们称之为：Short cut。
- shortcut路径大致也可以分成2种，取决于残差路径是否改变了feature map数量和尺寸，一种是将输入x原封不动地输出，另一种则需要经过1×1卷积来升维或者降采样，主要作用是将输出与F(x)路径的输出保持形状保持一致，两种结构如下图所示。
 ![](./img/ch4/identity.png)
 
### 4.7.3 Resnet的网络结构
​	下图为我们展示了一个34层的resnet网络，34层的平铺网络和vgg19网络三种不同的网络结构示意图，ResNet的设计有如下特点：
 ![](./img/ch4/resnet.png)
...
​	下表为我们展示了Resnet目前的主要网络结构，其中主要分为Resnet18,resnet34,resnet50,resnet101和resnet152这几种
![](./img/ch4/resnet_tabel.png)
- 我们以Resnet50为例讲解：其中中间的四个堆叠的残差块个数为（3,4,6,3）,对应每一的残差快由3个卷积组成，故一共有48个卷积层，加上输入的$7*7$，一共50层。
- 在每一个堆叠的卷积模块中，第一个是侧边带有卷积结构的conv_block,其中设置卷积核分别为(1x1,3x3,1x1)，中间3x3的卷积会加入降采样。接下来接侧边没有任何操作的identity_block

### 4.7.4 Imagenet测试结果
![](./img/ch4/resnet_results.png) 
...
## 4.8 为什么现在的CNN模型都是在GoogleNet、VGGNet或者AlexNet上调整的？
